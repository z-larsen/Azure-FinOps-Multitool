...
"@
    ...
    # Only treat as denied when nothing came back AND a query was forbidden.
    $denied = ($accessDenied -and $allRecommendations.Count -eq 0 -and $reservationRecs.Count -eq 0)
    $accessDeniedReason = $null
    if ($denied) {
        $accessDeniedReason = Get-FinOpsPermissionMessage -Capability 'Recommendations'
        Write-Host "  Recommendations: access denied reading Advisor / reservation recommendations." -ForegroundColor Yellow
    }
